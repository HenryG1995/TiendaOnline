...
export class SeguimientoEntregaComponent implements OnInit, AfterViewInit, OnDestroy {
  isLoading = false;
  ...
  displayedColumns: string[] = ['codigoventa', 'fechaventa', 'Estado', 'Acciones'];
  ...
  bitacora() {
    this.isLoading = true
    this.subscription?.push(
      this.ventaservice.seguimiento().subscribe(
        (response) => {
